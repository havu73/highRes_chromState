...
def open_one_enrichment_df(fn):
	...
	try:
		df = df.rename(columns={"state (Emission order)": "state", "Genome %": "fraction_in_bg", "State (User order)" : "state", 'State (Emission order)': 'state'})
	except:
...
def get_enrichment_data(train_test_enrich_folder_list):
	train_fn_list = list(map(lambda x: os.path.join(x, "train_overlap_against_bg.txt"), train_test_enrich_folder_list))
	test_fn_list = list(map(lambda x: os.path.join(x, 'test_overlap_against_bg.txt'), train_test_enrich_folder_list))
...
def do_roc_analysis(this_cont_df, train_cont_percent, test_cont_percent, enr_cont_name):
	"""
	consider 'gContext' as the analysis that we are talking about
	train_test_df_tuple: has 4 columns train_fraction_in_bg, test_fraction_in_bg, train_<cont_name>, test_<cont_name>
	"""
	# filter out state that is non existent on the genome
	this_cont_df = this_cont_df.loc[this_cont_df['test_fraction_in_bg'] > 0].copy()
	try:
		this_cont_df['state_spec_quantile'] = this_cont_df['train_state'].apply(lambda x: int(x.split('-')[1]))
		...
		this_cont_df.sort_values(by='state_spec_quantile', ascending = False, inplace = True)
		# higher quantile means higher posterior probs of being in this state
		# lower quantile means lower posterior probs of being in this state
	except:
		this_cont_df.sort_values(by='train_'+ enr_cont_name, ascending = False, inplace = True) # this is only applied in the case of the simple rule model where states are not in the form S<state_index>-<quantile index>
	# print(enr_cont_name)
	# print(this_cont_df.head())
	num_rows, num_cols = this_cont_df.shape # ncols should be 4: fraction_in_bg, genomic context of interest for train and test data
	...
	test_fract_bg_in_cont = test_cont_percent / 100.0 # fraction of the genome that the cont occupies
	test_fract_bg_not_cont = 1 - test_fract_bg_in_cont # fraction of the genome that the cont does not occupy
	this_cont_df['culm_frac_bg_in_state'] = (this_cont_df.test_fraction_in_bg).cumsum()
	this_cont_df['fract_cont_in_state'] = this_cont_df['test_fraction_in_bg'] * this_cont_df['test_' + enr_cont_name] # #MS/#M  = #S/#Gb * FE
	this_cont_df['fract_state_be_cont'] = this_cont_df['test_' + enr_cont_name] * test_fract_bg_in_cont # #MS/#S = FE * #M/#G
	...
	this_cont_df['fract_bg_in_state_and_cont'] = this_cont_df['fract_state_be_cont'] * this_cont_df['test_fraction_in_bg']  # #MS/#G = #MS/#S * #S/#G
	this_cont_df['culm_fract_bg_in_state_and_cont'] = (this_cont_df.fract_bg_in_state_and_cont).cumsum()
	this_cont_df['fract_bg_in_state_not_cont'] = this_cont_df['test_fraction_in_bg'] * (1 - this_cont_df['fract_state_be_cont']) # #S/#G * (1 - #MS/#S)
	this_cont_df['fract_not_cont_in_state'] = this_cont_df['fract_bg_in_state_not_cont'] / test_fract_bg_not_cont
	this_cont_df['true_pos'] = this_cont_df['culm_fract_bg_in_state_and_cont'] / test_fract_bg_in_cont
	this_cont_df['false_pos'] = (this_cont_df.fract_not_cont_in_state).cumsum()
	this_cont_df['precision'] = this_cont_df['culm_fract_bg_in_state_and_cont'] / this_cont_df['culm_frac_bg_in_state']
...
def write_results_data(outputFolder, enrichment_model_name, result_df_list, enr_cont_name, roc_or_precall):
	...
	print(os.path.join(outputFolder, enr_cont_name + DATA_FILE_SUFFIX_LIST[roc_or_precall]))
...
def get_train_test_one_enr_cont_df (train_test_tuple, enr_cont_name): 
	...
	columns_to_select = ['state', 'frac_bg_in_state_{cont}'.format(cont = enr_cont_name), enr_cont_name]
	train_df = ((train_test_tuple[TRAIN_INDEX])[columns_to_select][:-1]).copy() # we exclude the last row cuz those correspond to the row of percentage of genome that are in each of the enrichment context
	train_df = train_df.rename(columns = {enr_cont_name: 'train_' + enr_cont_name, 'frac_bg_in_state_{cont}'.format(cont = enr_cont_name) : "train_fraction_in_bg", 'state' : 'train_state'})
	num_train_state = train_df.shape[0] # number of states recorded in training data set
	test_df = ((train_test_tuple[TEST_INDEX])[columns_to_select][:-1]).copy()
	test_df = test_df.rename(columns = {enr_cont_name: 'test_' + enr_cont_name, 'frac_bg_in_state_{cont}'.format(cont = enr_cont_name) : "test_fraction_in_bg", 'state' : 'test_state'})
	num_test_state = test_df.shape[0] 
	# the folllowing line of codes are there to ensure that in cases where some states are not present in train and test datasets, then we will still account for those when we merge the train and test data into one df
	df = train_df.merge(test_df, how = 'outer', left_on = 'train_state', right_on = 'test_state', suffixes = ('',''))
	values = {'train_fraction_in_bg': 0, 'test_fraction_in_bg': 0, 'train_' + enr_cont_name: 0, 'test_' + enr_cont_name: 0}
	df = df.fillna(value = values)
	### end of merging step and dealing with missing states in either train or test data
	...
	return df, (train_cont_percent, test_cont_percent) # df will have the following columns: train_state, test_state, train_fraction_in_bg, test_fraction_in_bg, train_<cont>, test_<cont>. The train_fraction_in_bg and test_fraction_in_bg columns show the fraction in bg that each state occupies
...
def one_enr_cont_analysis(train_test_df_list, outputFolder,  enrichment_model_name, enr_cont_name):
	...
	for model_index, model_name in enumerate(enrichment_model_name):
		...
		# this_cont_df has 6 columns: train_state, test_state, train_fraction_in_bg, test_fraction_in_bg, train_<cont_name>, test_<cont_name>
		roc_df = do_roc_analysis(this_cont_df, train_cont_percent, test_cont_percent, enr_cont_name) # roc_df has 4 columns: false_pos, true_pos, precision, recall
		# print roc_df.head()
...
def create_roc_analysis(train_test_df_list, outputFolder, enrichment_model_name):
	...
	enr_cont_list = headers[1::2] # the first column is state, for all the following columns, they go in pairs (<cont>, frac_bg_in_state_<cont>) --> we only need to see the <cont> here
	for enr_cont_name in enr_cont_list: # enrichment analysis name is the name of genomic content that we want to do analysis of enrichment. Skip the first because it's state  and then every other colum names because those are the fract of genome in states--> irrelevant
...
def usage():
	...
	print ("train_test_enrich_folder_list: each item corresponds to a folder containing files train_overlap_against_bg.txt and test_overlap_against_bg.txt from one model")
