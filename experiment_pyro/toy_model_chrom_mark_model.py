...
def generate_data(N, num_state): # N: number of genomic positions
...
def model(alpha, bernouli_mark, ref_state_df, obs, num_obs, num_state, transiton_mat): # bernouli_mark: an array indexed by the state
	num_ct = len(alpha)
	pi = pyro.sample('pi', dist.Dirichlet(alpha))
	for i in pyro.plate('state_loop', num_state):
...
def guide(alpha, bernouli_mark, ref_state_df, obs, num_obs, num_state, transiton_mat):
	num_ct = len(alpha)
...
def train(alpha, bernouli_mark, ref_state_df, obs, num_state, transiton_mat):
	num_steps = 10
	...
	for _ in tqdm(range(num_steps)):
		loss = svi.step(alpha, bernouli_mark, ref_state_df, obs, num_obs, num_state, transiton_mat)
		losses.append(loss)
