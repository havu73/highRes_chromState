...
def get_parameters(parameter_folder, emission_fn, obs_chromMark_signal_fn):
	...
	emission_df = pd.DataFrame(emission_tt).astype(float)
	# emission_df.applymap(lambda x: x.item())
	emission_df.to_csv('/gstore/home/vuh6/source/train_model/toy_data/toy_output_read_emission_matrix_into_categorical_prob.txt', header = False, index = False, sep = '\t')
...
def prepare_ref_epig_state_df(all_refEpig_state_fn , ref_epig_name_list):
	ref_state_df = pd.read_csv(all_refEpig_state_fn, header = 0, index_col = None, sep = '\t')
	ref_state_df = ref_state_df[ref_epig_name_list] # select only columns corresponding to the reference epigenomes that are input to the model to predict the chromatin state maps in sample of interests. 
	ref_state_df.columns = np.arange(len(ref_epig_name_list)) # make all column names to be just 0-based indices of ref-Epig after they are ordered exactly as pi
	return ref_state_df
...
def calculate_P_state_and_mark(row, d4_tt, state_index, num_ct):
	...
	result = 0
	for refE_i in list(range(num_ct)): #summing over all reference epigenome
		R_Z = row[refE_i]
...
def main():
	...
	all_refEpig_state_fn  = sys.argv[3]
	helper.check_file_exist(all_refEpig_state_fn)
	...
	logger = helper.argument_log(sys.argv, os.path.dirname(output_fn), 'calculate_posteriorState') # to save the command line arguments that led to this 
	logger.write_log()
	...
	all_epig_df = prepare_ref_epig_state_df(all_refEpig_state_fn , ref_epig_name_list) # all_epig_df: rows: genomic positions (200bp), columns: reference epigenome ordered similarly to those in pi, column names are 0-based indices. values: 0-based state assignment at each genomic position at each ref_epig
...
def usage():
	...
	print ('all_refEpig_state_fn: where we store the data of reference epigenomes\' state maps in an entire table. This is applicable when running simulation. Columns: ref_epig_id, rows: genomic positons. values: 0-based integers indcating state assignments at each genomic position for each reference epigenome')
