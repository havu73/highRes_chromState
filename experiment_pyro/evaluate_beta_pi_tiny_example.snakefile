...
rule all:
	input:
		...
		expand(os.path.join(out_dir, 'no_beta_guide', 'extreme_control', 'n{NUM_TRAIN_ITERATIONS}_b{NUM_BINS_SAMPLE_PER_ITER}.txt'), NUM_TRAIN_ITERATIONS = NUM_TRAIN_ITERATIONS_list, NUM_BINS_SAMPLE_PER_ITER = NUM_BINS_SAMPLE_PER_ITER_list)
...
rule tiny_example_no_beta_guide:
	...
	shell:
		'''
			python tiny_toy_example_no_betaGuide.py -n {wildcards.NUM_TRAIN_ITERATIONS} -b {wildcards.NUM_BINS_SAMPLE_PER_ITER} -out {output[0]}
		'''

rule tiny_example_extreme_control:
	input:
	output:
		os.path.join(out_dir, 'no_beta_guide', 'extreme_control', 'n{NUM_TRAIN_ITERATIONS}_b{NUM_BINS_SAMPLE_PER_ITER}.txt')
	shell:
		'''
			python tiny_toy_extreme_control.py -n {wildcards.NUM_TRAIN_ITERATIONS} -b {wildcards.NUM_BINS_SAMPLE_PER_ITER} -out {output[0]}
		'''
