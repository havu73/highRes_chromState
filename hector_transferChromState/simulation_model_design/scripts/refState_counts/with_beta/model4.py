...
class Encoder(nn.Module):
	...
	def forward(self, m, r):
		...
		logpi_loc = self.bnmu(self.fcmu(h)) # num_states
		logpi_logvar = self.bnlv(self.fclv(h)) # num_states
...
class Decoder(nn.Module):
	...
	def forward(self, inputs):
		...
		ref_param = torch.sigmoid(self.bn_ref(self.fchr_ref(h_ref)))
...
class Model_four(nn.Module):
	...
	def generate_reconstructed_data(self, m, r):
		...
		re_r = pyro.sample('re_r', dist.Multinomial(self.num_ref_per_groups, ref_param).to_event(1))
		return(re_m, re_r)
